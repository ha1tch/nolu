...
// Demo 1 — Basic: three xolu instances, one organisation per instance.
//
// No persistent registry. Uses MemoryRegistry (state lost on exit).
// Organisations are modelled as separate sovereign xolu instances:
//
//   xolu-vendocorp   (port 9090) — VendoCorp, manufacturer
//   xolu-retailchain (port 9091) — RetailChain, vending operator
//   xolu-serviceco   (port 9092) — ServiceCo, repair provider
//
// Scenario:
//   1. VendoCorp registers 5 newly manufactured devices
//   2. Batch sale proposal + partial acceptance (devices 1–3 accepted, 4 rejected)
//   3. Cancellation (device 5 — quality hold)
//   4. Repair cycle: RetailChain → ServiceCo → RetailChain (device 2)
//   5. End-of-life retirement (device 3)
//   6. Final state snapshot
//
// Optional NATS (-bus=nats): all events flow through JetStream.
package main
